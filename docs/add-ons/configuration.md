...
| map | list | no | List of maps for additional Home Assistant folders to mount within the add-on. Possible values: `config`, `ssl`, `addons`, `backup`, `share` or `media`. Defaults to `ro`, which you can change by adding `:rw` to the end of the name. These folders are relative to the Supervisor directory. On Home Assistant OS, the Supervisor directory is at [`/mnt/data/supervisor/`](https://www.home-assistant.io/common-tasks/os/#configuring-access-to-files).
