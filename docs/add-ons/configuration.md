...
| snapshot | string | no | `hot` (default) or `cold`. If `cold`, the supervisor turn the add-on of before he take an snapshot and the options for `pre/post` are not working.
