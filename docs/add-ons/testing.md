...
- Copy the `.devcontainer` and `.vscode` folder from [Official Add-ons][hassio-addons] repository into the root of your "addons" folder.
- Open the "addons" folder inside VS Code, and when prompted re-open the window inside the container (or, from the Command Palette, select 'Rebuild and Reopen in Container'). 
...
- The add-on(s) created under your "addons" folder (i.e. addons/hello_world/) will automatically be found in the Local Add-ons repository.
