...
An entity is looked up in the registry based on a combination of the platform type (for example, `light`), and the integration name (domain) (for example, hue) and the unique ID of the entity. Entities should not include the `domain` (for example, `your_integration`) and platform type (for example, `light`) in their Unique ID as the system already accounts for these identifiers.
