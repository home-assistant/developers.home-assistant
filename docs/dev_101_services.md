...
[Service actions should always be registered in the integration's `async_setup` or `setup` function](integration-quality-scale/rules/action-setup), not in the integration's `async_setup_entry` or in a platform's `async_setup_entry`, `async_setup_platform` or `setup_platform`.
