...
:::info
If the platform of the entity is either `binary_sensor`, `number`, `sensor` or `update` and the entity has a device class set, and you want the entity to have the same name as the device class, you can omit the translation key because the entity will then automatically use the device class name.
:::
