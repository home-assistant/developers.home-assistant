...
meaning it has the same name as its device.
...
If there's a single main feature of a device it should typically have its name property return `None`.
...
This is an example of a switch entity which is the main feature of a device:

The example is using class attributes to implement properties, for other ways
...
This is an example of a switch entity which is either not the main feature of a device, or is not part of a device:
