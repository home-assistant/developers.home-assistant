...
| suggested_unit_of_measurement | string | `None` | The unit of measurement to be used for the sensor's state. For sensors with a `unique_id`, this will be used as the initial unit of measurement which users can then override. For sensors without a `unique_id`, this will be the unit of measurement for the sensor's state. This poperty is intended to be used by integrations to override automatic unit conversion rules, for example to make a temperature sensor always display in °C regardless of if the configured unit system prefers °C or °F.
