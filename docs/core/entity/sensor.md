...
| `SensorStateClass.MEASUREMENT_ANGLE` | Similar to the above `SensorStateClass.MEASUREMENT`, the state represents _a measurement in present time_ for angles measured in degrees (`°`). Example of what should be classified `SensorStateClass.MEASUREMENT_ANGLE` is: current wind direction
