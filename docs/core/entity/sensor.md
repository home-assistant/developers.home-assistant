...
| total | The state represents a total amount that can both increase and decrease, e.g. a net energy meter. Statistics of the accumulated growth or decline of the sensor's value since it was first added is updated every 5 minutes. This should not be used for sensors where the absolute value is interesting, for example remaining battery capacity or CPU load, in such cases `measurement` should be used instead.
