...
- [ ] 🤖 **Git history corruption.** PRs with merge commits from the base branch (instead of rebase) create tangled history that's impossible to review. If you see commits like "Merge branch 'dev' into feature-x", ask the author to rebase cleanly or open a fresh PR.
