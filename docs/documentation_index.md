...
**Note:**
Building the docs using `rake generate` can take a long time to finish (~21 minutes on a Celeron J3455 @ 1.5Ghz).
If you only need to generate the file you work on, use `rake isolate[name_of_your_markdown_file]`. 
Once you're done, use `rake integrate` to reverse the effects of `rake isolate`.
