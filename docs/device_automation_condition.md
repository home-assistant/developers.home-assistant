...
Device conditions can be provided by the integration that provides the device (for example, ZHA, deCONZ) or the entity integrations that the device has entities with (for example, light, humidity sensor).
