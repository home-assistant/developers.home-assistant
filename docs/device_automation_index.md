...
Device Automations provide users with a device-centric layer on top of the core concepts in Home Assistant. When creating automations, users no longer have to deal with core concepts like states and events. Instead, they will be able to pick a device and then pick from a list of pre-defined triggers, conditions and actions.
...
Device automations are not exposing extra functionality, but they are a way for users to not have to learn new concepts. Device automations are using events, state and service helpers under the hood.
