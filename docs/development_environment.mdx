...
If the Dev Container was setup correctly - it supports debugging by default, out-of-the-box. It provides the necessary debug configurations, so hitting F5 should launch Home Assistant and any breakpoints put in the code should be trigegred and the debugger should stop. 
