...
Using GitHub Codespaces you can start with development in an online environment. This approach does not allow for interaction with local hardware. The online environment is completely seperate from your local network.

**Prerequisites**
- [Git Account](https://github.com/join)
- A web-browser
- (Optional) [GitHub Account plan](https://docs.github.com/en/billing/managing-billing-for-github-codespaces/about-billing-for-github-codespaces)

**Getting started:**

1. Go to [Home Assistant core repository](https://github.com/home-assistant/core) and click "fork".
2. Once your fork is created, click the button "Code" and select the tab "Codespaces" 
3. Click "+" button to "Create a codespace on dev" branch
4. A new browser tab will be opened showing that the codespace is being setup (this may take a few minutes), after this your codespace will be ready.
5. A very basic configuration.yaml is created in the folder `/config`. We need to make a modification in this file to allow for using the builtin reverse-proxy.
    * Open the file `/config/configuration.yaml`
    * Append the following snippet to this file and save the modified file:
      ```
      http:
         ...
         trusted_proxies:
            - 127.0.0.1
      ```
5. You can verify that your environment is set up properly by the following:
    * Open the command palette in Visual Studio Code - `Shift`+`Command`+`P`(Mac) / `Ctrl`+`Shift`+`P` (Windows/Linux).
    * Select `Tasks: Run Task` -> `Run Home Assistant Core`
    * A terminal should open and begin outputting activity. Check for errors and wait for the output to stop/slow down.
    * A notification will be displayed giving the option to "Open in browser" or "Make public". Choose to "Open in browser".
    * You should see the Home Assistant setup screen. Continue the setup.
    * After clicking the "Finish" button, a browser error will probably be displayed. Modify the browser-url by removing `?auth_callback=....` and retry. 
    * The login screen of your online environment will now be displayed,
