...
4. When Visual Studio Code asks if you want to install the Dev Containers extension, click **Install**.
...
It is also possible to debug a remote Home Assistant instance (for example, production instance) by following the procedure described [here](https://www.home-assistant.io/integrations/debugpy/).
...
Drop a `default.nix` with the contents below in the checked out repository and activate it with, for example,
