...
```shell
brew install python3 autoconf ffmpeg rust openssl@1.1
```
...
on macOS with `brew`, use:
```shell
LDFLAGS="-L$(brew --prefix openssl@1.1)/lib" CFLAGS="-I$(brew --prefix openssl@1.1)/include" scripts/setup
```
