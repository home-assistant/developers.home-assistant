...
7. In order to get the right localization and therefore local time in the devcontainer you need to set or adapt the right timezone in the devcontainer.json under .devcontainer folder.
   Add  e.g. "TZ": "Europe/Berlin" to "containerEnv" section so that it looks like this (don't delete existing keys) and rebuild the container:
   "containerEnv": {
      "TZ": "Europe/Berlin"
    },
