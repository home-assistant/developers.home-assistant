...
6. You can verify that your dev container is set up properly by the following:
    * Open the command pallette in Visual Studio Code - `Shift`+`Command`+`P`(Mac) / `Ctrl`+`Shift`+`P` (Windows/Linux)
    * Select `Tasks: Run Task` -> `Run Home Assistant Core`
    * A terminal should open and begin outputting activity. Check for any errors and wait for output to stop/slow down
