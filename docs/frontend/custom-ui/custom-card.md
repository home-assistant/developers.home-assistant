...
Home Assistant will set the `hass` property when the state of Home Assistant changes (frequent). Whenever the state changes, the component will have to update itself to represent the latest state. The docs for the `hass` object can be found [here](https://developers.home-assistant.io/docs/frontend/data/).
