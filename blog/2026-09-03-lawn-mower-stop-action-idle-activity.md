...
`LawnMowerActivity.IDLE` describes a mower that is stopped but neither docked, paused nor in error. Integrations that mapped such a state to `PAUSED` or `ERROR` should now use `IDLE`.
